...
function scrapyardModifier(modifierLevel)
{
    var modifier;
    var level = Math.max(modifierLevel || 0, 1);
    if (level > 200)
      {
...
function achievementModifier(modifierLevel)
{
  var modifier;
  var amount = modifierLevel
  modifier = Math.max(0, 1000 - Math.max(0,amount * 2));
  return modifier;
}

function masteryBoost17Modifier(modifierLevel)
{
  var modifier;
  var amount = modifierLevel
...
export function runStarCalculations(data) {
  //const results = [];
  var gsAmount = 0;
  var magAmount = 0;
  var fragmentAmount = 0;
  var scrapyardMul = scrapyardModifier(data.inputs['modifierScrapyardInput']); // to be added
  var achievementMul = achievementModifier(data.inputs['modifierAchievementsInput']); // scaled by 1000, to be multiplied
  var masteryBoost17Mul = masteryBoost17Modifier(data.inputs['modifierMasteryInput']); // To be multiplied

  // Loop through 1–10 for star pairs
  for (let i = 1; i <= 10; i++) {
    const starVal = parseFloat(data.inputs["current-input-" + i]) || 0;
    const targetVal = parseFloat(data.inputs["target-input-" + i]) || 0;
  ...
  document.getElementById("gsCost").innerHTML = gsAmount;
  document.getElementById("magnetCost").innerHTML = magAmount;
  document.getElementById("fragmentCost").innerHTML = fragmentAmount;
  //console.log(results);
  //return results;
};
